...
public class Verify 
{
	public static void main(String[] args) throws IOException 
	{
		...
		for (int i=0;i<n;i++)
			for (int j=0;j<n;j++)
			{
				if (guest[i] != guest[j] && A[i][j] == 1) 
					System.out.println("guest["+ i +"] and guest["+ j +"] should be together");
				
				if (guest[i] == guest[j] && A[i][j] == -1) 
					System.out.println("guest["+ i +"] and guest["+ j +"] should be apart");
			}
		for (int i=0;i<m;i++)
		{
			int count = 0;
			for (int j=0;j<n;j++) 
				count = count + alloc[i][j];
			if (count != size) 
				System.out.println("table["+ i +"] is wrong size "+ count);
		}
		...
		for (int j=0;j<n;j++)
		{
			...
			for (int i=0;i<m;i++) 
				count = count + alloc[i][j];
			if (count != 1) 
				System.out.println("guest["+ j +"] must be in exactly 1 table");
		}
	}
}